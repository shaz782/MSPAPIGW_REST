 ...
 */
@Component
public class SuspendAccountImpl {
	
	private static final Logger LOGGER = LogManager.getLogger(SuspendAccountImpl.class);
	...
	public SuspendAccountResponse suspendAccount(SuspendAccountRequest request) throws MSPAPIGWExceptions {
		
		agentRequest = getAgentRequest(request, headers);
		smfRequest = getSmfAgentRequest(request, headers);
		if (smfRequest != null) {
			smfResponse = smfIvoker.transactionTracker(smfRequest, headers);
		} else {
			if (LOGGER.isErrorEnabled())
				LOGGER.error("SuspendAccountImpl-suspendAccount of TransactionTrackerRequest is Null");
		}
				...
				try {
					...
					if(agentResponse!=null && agentResponse.getRefTransId()!=null) {
						...
						if (trackerUpdateResponse == null) {
							LOGGER.error("SuspendAccountImpl-suspendAccount-SUCCESS- of TransactionTrackerUpdateResponse is Null");
						}
					...
					trackerUpdateResponse = smfIvoker.trackUpdate(trackUpdateRequest, headers);
					if (trackerUpdateResponse == null) {
							LOGGER.error("SuspendAccountImpl-suspendAccount-FAILURE- of TransactionTrackerUpdateResponse is Null");
					}
					...
					if (LOGGER.isErrorEnabled())
						LOGGER.error("SuspendAccountImpl-suspendAccount of UpdateActStatusRequest is Null");
			}
			}else {
				if (LOGGER.isErrorEnabled())
					LOGGER.error("SuspendAccountImpl-suspendAccount of TransactionTrackerResponse is Null");
	
			}
		return response;
	}

	private TransactionTrackerRequest getSmfAgentRequest(SuspendAccountRequest request, CommonHeaders headers2) {
		if (request != null) {
			...
			smfRequest.setActivityType("132"); // 132 --- SUSPEND ACCOUNT
	...
	private SuspendAccountResponse setAgentResponse(UpdateActStatusResponse agentResponse) {
	...
	private UpdateActStatusRequest getAgentRequest(SuspendAccountRequest request, CommonHeaders headers) throws  MSPAPIGWExceptions {
		...
		if (request != null) {
			...
			agentRequest.setOperationType(OperationType.DEACTIVATE);
