...
public class ActivateTPImpl {
		...
		try {
			...
			if (selfCareRespose != null && selfCareRespose.getFlag() != null
				...
				if (xBusResponse != null) {
					response = setAgentResponse(xBusResponse,selfCareRespose,activateTPRequest);
				} else {
	...
	private String smfIinsertUpdateTracker(ActivateTPRequest activateTPRequest, String flag, String remarks,
		...
		if (activateTPRequest != null && activateTPRequest.getLastName() != null) {
			...
			try {
				...
				smfReq.setLastName(lastNmae);
				LOGGER.info("---------------------- spanish value  >"+ lastNmae);
				LOGGER.info("---------------------- spanish value  >"+ smfReq.getLastName());
	...
	private GetIDvalueBylastDigitsResponse checkIDValue(ActivateTPRequest activateTPRequest) throws MSPAPIGWExceptions {
		...
		if (respose != null) {
			if (respose.getIDValue() != null) {
				...
				smfIinsertUpdateTracker(null, "U", "Given IDValue Not Found", transID);
	...
	private ActivateTPResponse setAgentResponse(ActivateTPtoSubscriberResponse xBusResponse, GetIDvalueBylastDigitsResponse selfCareRespose, ActivateTPRequest activateTPRequest) throws MSPAPIGWExceptions {
		...
		if (xBusResponse != null) {
			...
			if (xBusResponse.getStatusCode() != null
				...
				if(selfCareRespose.getIDValue()!=null) {
					response.setIdValue(selfCareRespose.getIDValue());
				}if(activateTPRequest.getIdType()!=null)
					response.setIdType(activateTPRequest.getIdType());
