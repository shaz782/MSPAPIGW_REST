...
import com.xius.msp.billing.messages.balanceManagement.DoRechargeRequest;
import com.xius.msp.billing.messages.balanceManagement.DoRechargeResponse;
...
import com.xius.msp.billing.messages.balanceManagement.GetSubsDetailsRequest;
import com.xius.msp.billing.messages.balanceManagement.GetSubsDetailsResponse;
 ...
 * 
 * @author shashidhar.p
 * @since 17th jan,2020 Description : From Here we will invoke the
 *        InfinetBilling Services.
 */
@Component
public class BalanceManagementAgentInvoker {
	...
	public GetAvailableDenomsResponse getAvailableDenoms(GetAvailableDenomsRequest agentRequest, CommonHeaders headers)
		...
		try {
			bindingStub = new BalanceManagementBindingStub(new URL(getBalMgmtURL()), new BalanceServiceLocator());
	...
	public RechargeResponse recharge(RechargeRequest agentRequest, CommonHeaders headers) throws MSPAPIGWExceptions {
	...
	public GetSubsDetailsResponse getSubsDetails(GetSubsDetailsRequest getSubsDetailsRequest, CommonHeaders headers)
			throws MSPAPIGWExceptions {

		if (LOGGER.isInfoEnabled()) {
			LOGGER.info("BalanceManagementAgentInvoker - GetSubsDetails called");
		}

		GetSubsDetailsResponse GetSubsDetailsResponse = new GetSubsDetailsResponse();
		BalanceManagementBindingStub bindingStub = null;
		...
		try {
			...
			GetSubsDetailsResponse = bindingStub.getSubsDetails(getSubsDetailsRequest);

		} catch (ErrorDetailsType e) {
			if (LOGGER.isErrorEnabled())
				LOGGER.error("BalanceManagementAgentInvoker - GetSubsDetails-ErrorDetailsType" + e);
			throw new MSPAPIGWExceptions(e.getErrorCode(), e.getErrorMessage());
		} catch (Exception e) {
			if (LOGGER.isErrorEnabled())
				LOGGER.error("BalanceManagementAgentInvoker - GetSubsDetails-Exception" + e);
			throw new MSPAPIGWExceptions(MSPAPIGWConstants.INTERNAL_SERVER_ERROR,
					MSPAPIGWConstants.CONNECTION_TIME_OUT_MSG);
		}

		return GetSubsDetailsResponse;

	}

	public DoRechargeResponse doRecharge(DoRechargeRequest DoRechargeRequest, CommonHeaders headers)
			throws MSPAPIGWExceptions {

		if (LOGGER.isInfoEnabled()) {
			LOGGER.info("BalanceManagementAgentInvoker - doRechargeCheck called");
		}

		DoRechargeResponse DoRechargeResponse = new DoRechargeResponse();
		BalanceManagementBindingStub bindingStub = null;
		...
		try {
			...
			DoRechargeResponse = bindingStub.doRecharge(DoRechargeRequest);

		} catch (ErrorDetailsType e) {
			if (LOGGER.isErrorEnabled())
				LOGGER.error("BalanceManagementAgentInvoker - doRechargeCheck-ErrorDetailsType" + e);
			throw new MSPAPIGWExceptions(e.getErrorCode(), e.getErrorMessage());
		} catch (Exception e) {
			if (LOGGER.isErrorEnabled())
				LOGGER.error("BalanceManagementAgentInvoker - doRechargeCheck-Exception" + e);
			throw new MSPAPIGWExceptions(MSPAPIGWConstants.INTERNAL_SERVER_ERROR,
					MSPAPIGWConstants.CONNECTION_TIME_OUT_MSG);
		}

		return DoRechargeResponse;
