...
@Component
public class ActivateRcProductImpl {
	
	private static final Logger LOGGER = LogManager.getLogger(ActivateRcProductImpl.class);
	...
	ActivateRcProductRequest xBusRequest=null;
	ActivateRcProductResponse xBusResponse = null;
	...
	public RechargeWithCCResponse activateRcProduct(RechargeWithCCRequest rechargeWithCCRequest) throws MSPAPIGWExceptions {
		...
		if (xBusRequest != null) {
			...
			if (LOGGER.isErrorEnabled())
				LOGGER.error("ActivateRcProductImpl-activateRcProduct of xBus RechargeWithCcRequest is Null");
		}
		if (xBusResponse != null) {
			response = setAgentResponse(xBusResponse);
		} else {
			if (LOGGER.isErrorEnabled())
				LOGGER.error("ActivateRcProductImpl-activateRcProduct of xBus RechargeWithCcResponse is Null");
		}
		return response;
	}
	
	
	private RechargeWithCCResponse setAgentResponse(ActivateRcProductResponse xBusResponse) throws MSPAPIGWExceptions {
			...
			response = new RechargeWithCCResponse();
				...
				if(xBusResponse.getStatusCode()!=null &&( xBusResponse.getStatusCode().getValue().equalsIgnoreCase("success") ||
					...
					if (LOGGER.isInfoEnabled())
						LOGGER.info( "xBusDebitOrCreditInvoker  ActivateRcProductResponse Status Code:"+xBusResponse.getStatusCode().getValue());
				...
				if(xBusResponse.getStatusCode().getValue().equalsIgnoreCase("ERROR")){
					...
					String errorMessage = null;
						...
						if (LOGGER.isInfoEnabled())
							LOGGER.info( "xBusDebitOrCreditInvoker  ActivateRcProductResponse Error Code :"+errorCode);
						if (LOGGER.isInfoEnabled())
							LOGGER.info("xBusDebitOrCreditInvoker  ActivateRcProductResponse Error Msg :"+errorMessage);
	...
	private ActivateRcProductRequest getAgentRequest(RechargeWithCCRequest request, CommonHeaders headers) throws MSPAPIGWExceptions{
		...
		if (request != null ) {
			...
			if (mdnVsAccIDValidate(request, headers)) {
				xBusRequest = new ActivateRcProductRequest();
				...
				if (request.getRechargeAmount() != null) {
					ETopUpDetailsType eDetails = new ETopUpDetailsType();
					eDetails.setReChargeId(utility.getProperty("rcString") + request.getRechargeAmount());
					xBusRequest.setETopUp(eDetails);
				}
				...
				if (request.getAuthorizationID() != null) {
					xBusRequest.setAuthorizationID(request.getAuthorizationID());
				}
	...
	private boolean mdnVsAccIDValidate(RechargeWithCCRequest request, CommonHeaders headers) throws MSPAPIGWExceptions {
		...
		if (LOGGER.isInfoEnabled())
			LOGGER.info("ActivateRcProductImpl  mdnVsAccIDValidate  matheod called");
		...
		if ((request.getMsisdn() != null && request.getAccountId() != null)) {
			...
			if (request.getMsisdn() != null) {
				...
				if (LOGGER.isInfoEnabled())
					LOGGER.info(
							"activateRcProductImpl  mdnVsAccIDValidate- Request Details: " + agentRequest.getMSISDN());
			}

			agentResponse = accountManagementInvoker.mdnBasedGetNwId(agentRequest, headers);
			if (agentResponse != null) {
				if (LOGGER.isInfoEnabled())
					LOGGER.info("ActivateRcProductImpl  mdnVsAccIDValidate- Response Details For : "
				...
				if (LOGGER.isErrorEnabled())
					LOGGER.error("ActivateRcProductImpl  mdnVsAccIDValidate- Response is Null");
				return false;
			}
