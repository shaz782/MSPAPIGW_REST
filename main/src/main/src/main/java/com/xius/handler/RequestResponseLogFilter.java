...
public class RequestResponseLogFilter extends OncePerRequestFilter {
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
		...
		if (validNetwork == 1 || validNetwork == 3) { // authentication Success or authentication is off
			if (validNetwork == 3) {
				logger.info("RequestResponseLogFilter.doFilterInternal##validNetwork Not Configured");
			} else {
				logger.info("RequestResponseLogFilter.doFilterInternal##validNetwork SUCCESS");
			}
		} else if (validNetwork == 2) { // Exception occurred during authentication process
			logger.error("AxisFault Exception Occured During authentication process");
			((HttpServletResponse) wrapperResponse).sendError(HttpServletResponse.SC_UNAUTHORIZED,
					"NetWork Authentication Failed.");
		}

		int authResponseflag = authenticate(cachedBodyHttpServletRequest);

		if (authResponseflag == 0 || authResponseflag == 3) { // authentication Success or authentication is off
			if (authResponseflag == 3) {
				logger.info("RequestResponseLogFilter.doFilterInternal##Authentcation Not Configured");
			} else {
				logger.info("RequestResponseLogFilter.doFilterInternal##Authentcation SUCCESS");
			}
			filterChain.doFilter(cachedBodyHttpServletRequest, responseCopier);
		} else if (authResponseflag == 2) { // Exception occurred during authentication process
			logger.error("AxisFault Exception Occured During authentication process");
			((HttpServletResponse) wrapperResponse).sendError(HttpServletResponse.SC_UNAUTHORIZED,
	...
	private int checkNetworkId(CachedBodyHttpServletRequest request) {
		...
		try {
			...
			String mspApi = splitString[splitString.length - 3];
			String apiName = splitString[splitString.length - 1];
			logger.info("API NAME:" + apiName);

			if (mspApi != null && !mspApi.equalsIgnoreCase("MSP")) {
				return validNw;
			} else {
				JSONObject myResponse = new JSONObject(payload);
				String nwId = myResponse.get("carrierID").toString();
				...
				if (mspApi.equalsIgnoreCase("MSP") && apiName.equalsIgnoreCase("logIn")) {
					...
					utility = webApplicationContext.getBean(Utility.class);

					String authSwitch = utility.getProperty("networkId_check");

					if (authSwitch != null && authSwitch.equalsIgnoreCase(nwId)) {
						return validNw;
					} else if (authSwitch != null && authSwitch.equalsIgnoreCase("-1")) {
						return validNw = 3;
					} else if (authSwitch != null) {
						return validNw = 2;
					}
				}
			}
		} catch (AxisFault e) {
			logger.error("validNw - AxisFault >>>> " + e);
			validNw = 2;
		} catch (IOException e) {
			logger.error("validNw - IOException >>>> " + e);
			validNw = 2;
		} catch (Exception e) {
			logger.error("validNw - Exception >>>> " + e);
			validNw = 2;
		}
		return validNw;
	}
